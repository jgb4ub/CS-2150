...
bool hashTable::hashCheck(char* c){
  ...
  //stack<string>currList = hashVect.at(index);
...
int hashTable::hash(string c){
  int powerArr[] = {17,289,4913,83521,1419857,24137569};
  int intVal= 0;
  
  //cout<< c<< " is hash input" <<endl;
  for (int i=0; i<c.length();i++){
    if (c[i]!= '&' && c[i]!= '\'' && c[i]!= '-')
      //intVal+= (((int)c[i])*pow(17,i));
       intVal+= (int)c[i]*(powerArr[i%6]);
  }
